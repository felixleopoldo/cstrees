...
def test_predict():
    ...
    # test empty
    empty_outcome = {}
    assert t.predict(empty_outcome) == (2, 0, 1, 1)
    assert t.predict(empty_outcome, True) == ((2, 0, 1, 1), 0.11590809641032397)

    # test complete
    complete_outcome = {idx: outcome for idx, outcome in enumerate((2, 0, 1, 1))}
    assert t.predict(complete_outcome, True) == ((2, 0, 1, 1), 1.0)

    # partials
    partial_outcome_1 = {0: 1}
    t.predict(partial_outcome_1, True)
    assert t.predict(partial_outcome_1, True) == ((1, 0, 1, 1), 0.26674411494959)

    partial_outcome_2 = {0: 1, 3: 2}
    assert t.predict(partial_outcome_2, True) == ((1, 1, 1, 2), 0.3829026812192928)

    partial_outcome_3 = {0: 1, 2: 0, 3: 2}
    assert t.predict(partial_outcome_3, True) == ((1, 0, 0, 2), 0.5913854582044948)
